...
| El orden de las acciones, **declarado al revés** porque VoiceOver las lee al revés | Se oyen en el orden que se quiere | TalkBack las ofrece **en el orden declarado**, una vez cada una (medido el 2026-09-23). Se declaran en el orden en que se quieren oír | Sí, y más sencillo |
...
| Devolver el foco al cerrar un diálogo o al desaparecer una fila (lo hacía la de Expo con `sendAccessibilityEvent`; la de iOS lo deja a SwiftUI) | No perder el sitio | Compose puede mover el **foco de entrada** con `FocusRequester.requestFocus()`, y TalkBack suele llevar su cursor a lo que recibe el foco de entrada, pero **no siempre**: son dos focos distintos y no hay API pública de Compose para mover el de accesibilidad. Medido el 2026-09-23: **TalkBack no sigue a `requestFocus()`**, se iba al título. Lo que funciona es pedir `ACTION_ACCESSIBILITY_FOCUS` al proveedor de accesibilidad de la vista (`CursorDeTalkBack.kt`): el cursor va a la fila siguiente, o a la anterior, y al cancelar vuelve a la misma | No directo; alternativa comprobada |
